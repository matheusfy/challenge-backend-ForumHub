...
public record RespostaDetailDto(Long id, String mensagem, String solucao, String nomeUsuario, Long idTopico,
    String tituloTopico,
  ...
  public RespostaDetailDto(Resposta resp) {
    this(resp.getId(), resp.getMensagem(), resp.getSolucao(), resp.getUsuario().getNome(),
        resp.getTopico().getId(), resp.getTopico().getTitulo(),
        resp.getDataCriacao().format(DateTimeFormatterConfiguration.formatter));
  }
}
